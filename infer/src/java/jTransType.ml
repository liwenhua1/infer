...
let get_class_type_no_pointer program tenv cn =
  ...
  Typ.mk_struct (typename_of_classname (cn:JBasics.class_name))
...
let return_type program tenv ms =
  match JBasics.ms_rtype ms with None -> StdTyp.void | Some (vt:JBasics.value_type) -> value_type program tenv vt 
