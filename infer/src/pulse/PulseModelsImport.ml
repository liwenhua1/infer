...
module Basic = struct
  ...
  let err_cast_abort (name:Typ.Name.t) (target:Typ.Name.t) num_instance trace loc post ap_before p_or_re: (AbductiveDomain.t execution_domain_base_t, base_error) pulse_result list= [FatalError (
                                    ReportableError
                                 { diagnostic=
                                     JavaCastError
                                       {calling_context= [] ;
